...
# Compare SS1-6 to the average of EC1-EC8
...
# check order
all(pheno$sample == c("EC1","EC2","EC3","EC5","EC6","EC7","EC8","SS1","SS2","SS3","SS4","SS5","SS6"))
all(colnames(counts) == c("EC1","EC2","EC3","EC5","EC6","EC7","EC8","SS1","SS2","SS3","SS4","SS5","SS6"))
...
write.table(up, file='data/DEresults/MMP8p_vs_EC/up.csv', row.names = FALSE, quote = FALSE, col.names = FALSE)

dn <- rownames(deseq)[which(deseq$log2FoldChange < -1.5 & deseq$padj < .05 & cfc$FCdn)]
write.table(dn, file='data/DEresults/MMP8p_vs_EC/down.csv', row.names = FALSE, quote = FALSE, col.names = FALSE)
